...
spam(){
    ...
    if [ $pilih = "1" ]; then
            ...
            echo contoh 081553222878
            read target # masukin no telp
            echo
            echo "Berapa sms yang mau dikirim?"
            read paket
            echo
            echo Apakah anda yakin akan Troll Spam ke nomor $target "dan SMS dikirim "$paket"?"
            echo y/n?
            read confirm
            echo
            if [ $confirm = "y" ]; then
...
clear
troll
echo Selamat datang....., Masukkan nick Anda? #tulisan keluar
